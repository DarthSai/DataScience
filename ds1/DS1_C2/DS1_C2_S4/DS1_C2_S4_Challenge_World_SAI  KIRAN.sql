select country.name, code from country where (code='USA' or code='GBR' or code='ARE');
select avg(cc) from(select continent,sum(population) as cc from country group by continent) as c;
select country.name from country join countrylanguage on country.code=countrylanguage.countrycode where country.continent='Europe' and countrylanguage.Language='French';
select continent,population,name from country where (population,continent) in (select max(population),continent from country group by continent) order by population desc;
