...
select concat(employees.first_name,' ',employees.last_name),salary from employees where salary>(select avg(salary) from employees);
select concat(employees.first_name,' ',employees.last_name),salary from employees where( salary<(select avg(salary) from employees) and job_id like 'SA%');
select concat(first_name,' ',last_name) as 'Employee Name',salary from employees where salary<(select avg(salary) from employees where job_id='IT_PROG');
select first_name,last_name,salary from employees where salary=(select min(salary) from employees);
select concat(first_name,' ',last_name) from employees where salary>0.6*(select sum(salary) from employees ee where employees.department_id=ee.department_id); 
select first_name,last_name from employees join departments on employees.department_id=departments.department_id join locations on departments.location_id=locations.location_id join countries on locations.country_id=countries.country_id where(employees.manager_id is not null and countries.country_name='United Kingdom'); 
select first_name,last_name,salary into outfile 'C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\task8.csv' fields terminated by',' from employees order by salary desc limit 5;
