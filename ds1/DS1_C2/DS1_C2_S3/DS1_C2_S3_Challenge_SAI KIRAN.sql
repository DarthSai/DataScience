/*Task 1*/
select locations.city,count(employees.employee_id) from employees join departments on employees.department_id=departments.department_id join locations on departments.location_id=locations.location_id group by city order by count(employees.employee_id) desc;

/*Task 2*/
with c as 
(
select departments.department_id, locations.city,count(employees.employee_id) from employees join departments on employees.department_id=departments.department_id join locations on departments.location_id=locations.location_id group by city order by count(employees.employee_id) desc limit 5
)
select first_name,last_name,co.country_name,c.city,d.department_name,salary,co.country_name from locations l join departments d on d.location_id=l.location_id
join c on c.department_id=d.department_id
join employees e on e.department_id=d.department_id
join locations lo on d.location_id=l.location_id
join countries co on lo.country_id=co.country_id;

/*Task 3*/
select locations.city,count(employees.employee_id) from employees join departments on employees.department_id=departments.department_id join locations on departments.location_id=locations.location_id group by city order by count(employees.employee_id) desc limit 5;

/*Task 4*/
select employee_id,first_name,last_name,email,phone_number,(year(curdate())-year(hire_date)) as 'Experience' from employees join departments on employees.department_id = departments.department_id where (year(curdate())-year(hire_date))>=10;

/*Task 5*/
with z as
(
select employees.manager_id ee from employees 
join departments on employees.department_id = departments.department_id where (year(curdate())-year(hire_date))>=10
)
select concat(employees.first_name,' ',employees.last_name) as 'Manager Name' ,employee_id as 'Manager Employee ID' from employees 
join z on z.ee=employees.manager_id;

/*Task 6*/
select countries.country_name,avg(year(curdate())-year(hire_date)) as 'Average years spent in organisation' from countries 
join locations on countries.country_id=locations.country_id join departments on locations.location_id=departments.location_id 
join employees on departments.department_id=employees.department_id group by countries.country_name
order by avg(year(curdate())-year(hire_date)) desc limit 3;
