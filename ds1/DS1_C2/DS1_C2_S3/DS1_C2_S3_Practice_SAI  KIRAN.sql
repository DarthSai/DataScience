select employees.first_name ,employees.last_name,employees.employee_id,departments.department_name from employees join departments on employees.department_id=departments.department_id where departments.department_name='IT';
select employees.first_name ,employees.job_id,jobs.job_title,jobs.min_salary,jobs.max_salary from employees left outer join jobs on employees.job_id=jobs.job_id;
select count(employees.employee_id),locations.city from employees join departments on employees.department_id=departments.department_id join locations on departments.location_id=locations.location_id group by city order by count(employees.employee_id) desc limit 10;
select employees.employee_id,employees.first_name,employees.last_name from employees join job_history on employees.employee_id=job_history.employee_id where job_history.end_date='1999-12-31';
select employees.employee_id,employees.first_name,departments.department_name,(year(curdate())-year(hire_date)) as 'Experience' from employees join departments on employees.department_id = departments.department_id where (year(curdate())-year(hire_date))>=25;
