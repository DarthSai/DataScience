select count(employee_id) from employees;        /*Task 1*/
select department_id, count(employee_id) from employees group by(department_id);        /*Task 2*/
select first_name,salary from employees where salary>6000;        /*Task 3*/
select count(employee_id) from employees where salary>20000;        /*Task 4*/
select* from employees where( (commission_pct is not null) and commission_pct>0);        /*Task 5*/
select concat(first_name,' ',last_name) as Name from employees where commission_pct is null;
 select employee_id as 'Employee ID',concat(first_name,' ',last_name) as 'Employee Full Name',email as 'Email ID',phone_number as 'Contact Number' from employees where commission_pct is not null;        /*Task 7*/
select department_id,avg(salary) from employees group by(department_id) order by avg(salary) limit 3;      /*Task 8*/
select* from employees where job_id like '%CLERK';        /*Task 9*/
select count(employee_id),avg(salary) from employees where job_id like '%CLERK';        /*Task 10*/
select department_id,count(employee_id) from employees where salary between 7000 and 10000 group by department_id;        /*Task 11*/
